...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const dayOfWeek = body?.dayOfWeek ?? body?.day_of_week;
    const mealIds = body?.mealIds ?? body?.meal_ids ?? [];

    if (!isValidDayOfWeek(dayOfWeek)) throw new Error('Invalid dayOfWeek');
    ...
    const { error: deleteError } = await supabase.from('weekday_menus').delete().eq('day_of_week', dayOfWeek);
    if (deleteError) throw new Error(deleteError.message);

    if (mealIds.length > 0) {
      const inserts = mealIds.map((id: string) => ({ day_of_week: dayOfWeek, meal_id: id }));
